...
namespace WaveletsOnInterval
{
	...
	struct FullTransformEngine : WaveletSweep< ChosenWavelets, ROWSIZE, COLSIZE>
	{
		...
		int threshold(const FwtAp eps, std::bitset<REFBS * REFBS * REFBS>& mask_survivors, DataType * const buffer_survivors, const FwtAp data[SLICESIZE][COLSIZE][ROWSIZE])
		{
			enum { BSH = BS / 2 };
		...
		void load(std::vector<DataType>& datastream, std::bitset<BS * BS * BS> mask, FwtAp data[SLICESIZE][COLSIZE][ROWSIZE])
		{
			...
			//code 0
			{
				std::bitset<BSH * BSH * BSH> childmask;
	...
	struct FullTransform : FullTransformEngine<BS, BS, BS, BS>
	{
		...
		template<typename DataType, int REFBS>
		int threshold(const FwtAp eps, std::bitset<REFBS * REFBS * REFBS>& mask_survivors, DataType * const buffer_survivors)
		{
			return FullTransformEngine<BS, BS, BS, BS>::template threshold<DataType, BS>(eps, mask_survivors, buffer_survivors, data);
		}

		template<typename DataType>
		void load(std::vector<DataType>& datastream, std::bitset<BS * BS * BS> mask)
		{
